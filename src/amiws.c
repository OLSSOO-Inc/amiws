...
/**
 * @modifer norman namkoong <norman@olssoo.com>
 */
...
#define DEFAULT_PID_FILE    "/tmp/cmiws.pid"
                      ...
                      const char *wdir)
{
  ...
  syslog (LOG_INFO, "Daemonizing cmiws process.");
...
static void usage()
{
  ...
  printf("Communications Management Interface (CMI) to WebSocket converter.\n\n");

  printf("Options:\n");
  printf("-h, --help              Printf this help message.\n");
  printf("-f, --file FILENAME     Configuration file. By default '/etc/cmiws.yaml'\n");
  printf("-d, --daemon            Daemonize process.\n");
  printf("-p, --pidfile FILENAME  PID file when run as daemon. By default '/tmp/cmiws.pid'\n");
